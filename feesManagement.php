    ...
    try {
        ...
        $removedIdsSql = "
            ...
            WHERE enrolment_id = ?
              ...
              AND instalment_label NOT IN (" . implode(',', array_fill(0, count($keepLabels), '?')) . ")
        ";
        $removedIdsStmt = $pdo->prepare($removedIdsSql);
        $removedIdsStmt->execute(array_merge([$enrolmentId], $keepLabels));
        $removedIds = array_map('intval', $removedIdsStmt->fetchAll(PDO::FETCH_COLUMN));
        $removedBefore = [];
        foreach ($removedIds as $rid) {
            $removedBefore[$rid] = bbcc_fee_payment_snapshot($pdo, $rid);
        }
        ...
        foreach ($removedIds as $rid) {
            bbcc_audit_fee_payment_change($pdo, $rid, 'removed_start_term_adjust', $removedBefore[$rid] ?? null, null);
        }
        ...
        if ($plan === 'Yearly') {
            $yearlyIdStmt = $pdo->prepare("
                SELECT id FROM pcm_fee_payments
                WHERE enrolment_id = :eid AND plan_type = 'Yearly' AND instalment_label = 'Yearly'
                LIMIT 1
            ");
            $yearlyIdStmt->execute([':eid' => $enrolmentId]);
            $yearlyId = (int)($yearlyIdStmt->fetchColumn() ?: 0);
            $yearlyBefore = $yearlyId > 0 ? bbcc_fee_payment_snapshot($pdo, $yearlyId) : null;
            ...
            if ($yearlyId > 0) {
                bbcc_audit_fee_payment_change(
                    ...
                    'start_term_adjust',
                    $yearlyBefore,
                    bbcc_fee_payment_snapshot($pdo, $yearlyId)
                );
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['class_charge_action'])) {
    try {
        ...
        if ($act === 'add') {
            ...
            $syncIdsStmt = $pdo->prepare("
                SELECT id FROM pcm_fee_payments
                WHERE class_charge_id = :ccid AND status IN ('Unpaid', 'Pending')
            ");
            $syncIdsStmt->execute([':ccid' => $chargeId]);
            $syncIds = array_map('intval', $syncIdsStmt->fetchAll(PDO::FETCH_COLUMN));
            $syncBefore = [];
            foreach ($syncIds as $sid) {
                $syncBefore[$sid] = bbcc_fee_payment_snapshot($pdo, $sid);
            }
            ...
            foreach ($syncIds as $sid) {
                bbcc_audit_fee_payment_change(
                    ...
                    'class_charge_sync',
                    $syncBefore[$sid] ?? null,
                    bbcc_fee_payment_snapshot($pdo, $sid)
                );
            }
