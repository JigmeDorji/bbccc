...
try {
    ...
    ========================= */
    $sqlUpcoming = "SELECT * FROM events WHERE event_date >= :today";
    $paramsUpcoming = [
        ':today' => $today
    ];

    if (!empty($filterYear)) {
        $sqlUpcoming .= " AND YEAR(event_date) = :yr";
        $paramsUpcoming[':yr'] = (int)$filterYear;
    }

    if ($filterMonth !== '') {
        $sqlUpcoming .= " AND MONTH(event_date) = :mo";
        $paramsUpcoming[':mo'] = (int)$filterMonth;
    }

    if ($search !== '') {
        $sqlUpcoming .= " AND (title LIKE :s OR description LIKE :s2)";
        $paramsUpcoming[':s']  = "%$search%";
        $paramsUpcoming[':s2'] = "%$search%";
    }

    $sqlUpcoming .= " ORDER BY event_date ASC, start_time ASC";
...
<head>
    ...
    <style>
        ...
        .ev-filter select:focus, .ev-filter input:focus {
            ...
            box-shadow: 0 0 0 3px rgba(136,27,18,.08);
        }
        .ev-filter .btn-group {
            display: flex;
            gap: 8px;
...
<section class="bbcc-section">
    <div class="bbcc-container">
        ...
        <div class="section-header fade-up" style="margin-bottom:40px;">
            ...
            <h2>BBCC Events <span>Calendar <?= htmlspecialchars($calYear) ?></span></h2>
        ...
        <form method="GET" class="ev-filter fade-up">
            ...
            <div class="fg">
                ...
                <select name="month">
                    ...
                    <?php for ($m = 1; $m <= 12; $m++): ?>
                        <option value="<?= $m ?>" <?= ($filterMonth !== '' && (int)$filterMonth === $m) ? 'selected' : '' ?>>
        ...
        <div class="view-btns">
            <button class="active" id="btnList" type="button"><i class="fa-solid fa-list"></i> List</button>
            <button id="btnCal" type="button"><i class="fa-solid fa-calendar"></i> Calendar</button>
        </div>

        <!-- LIST VIEW -->
        <div id="listView">

            <h3 class="ev-section-title">Upcoming Events</h3>

            <?php if (empty($upcomingEvents)): ?>
                <div style="text-align:center;padding:40px 0;">
                    <i class="fa-regular fa-calendar-xmark" style="font-size:3rem;color:var(--gray-400);"></i>
                    <p style="margin-top:16px;color:var(--gray-600);">No upcoming events available right now.</p>
                </div>
            <?php else: ?>
                        ...
                        <div class="ev-card__body">
                            <div class="ev-card__info">
                                ...
                                <div class="ev-card__meta">
                                    <span>
                                        ...
                                        <?= !empty($ev['start_time']) ? date('h:i A', strtotime($ev['start_time'])) : 'TBA' ?>
                                        <?= !empty($ev['end_time']) ? ' to ' . date('h:i A', strtotime($ev['end_time'])) : '' ?>
                                    </span>
                            ...
                            <div class="ev-card__actions">
                                ...
                                <?php if (!$isBooked && $ev['status'] === 'Available'): ?>
                                    <a href="book-event?id=<?= (int)$ev['id'] ?>" class="bbcc-btn bbcc-btn--primary bbcc-btn--sm">
            ...
            <?php if (empty($pastEvents)): ?>
                <div style="text-align:center;padding:40px 0;">
                    <i class="fa-regular fa-calendar" style="font-size:3rem;color:var(--gray-400);"></i>
                    <p style="margin-top:16px;color:var(--gray-600);">No past events found.</p>
                </div>
            <?php else: ?>
        ...
        <div id="calView" style="display:none;">
            <div class="cal-nav">
                ...
                ?>
                <a href="events?year=<?= $prevY ?>&month=<?= $prevM ?>&search=<?= urlencode($search) ?>" class="bbcc-btn bbcc-btn--outline bbcc-btn--sm">&laquo; Prev</a>
                <h3><?= $monthName ?> <?= $calYear ?></h3>
                <a href="events?year=<?= $nextY ?>&month=<?= $nextM ?>&search=<?= urlencode($search) ?>" class="bbcc-btn bbcc-btn--outline bbcc-btn--sm">Next &raquo;</a>
            </div>
                                ...
                                <td class="<?= $isToday ? 'today' : '' ?>">
                                    ...
                                    <?php if (isset($calendarEvents[$dateStr])): ?>
                                        <?php foreach ($calendarEvents[$dateStr] as $cev): ?>
                                            <?php
                                            $evClass = 'avail';
                                            if ($cev['status'] === 'Pending Approval') {
                                                $evClass = 'pend';
                                            } elseif (!empty($cev['sponsors']) || $cev['status'] === 'Booked') {
                                                $evClass = 'booked';
                                            }
                                            ?>
                                            <a href="book-event?id=<?= (int)$cev['id'] ?>"
                                               class="cal-ev <?= $evClass ?>"
                                               title="<?= htmlspecialchars($cev['title']) ?> (<?= htmlspecialchars($cev['status']) ?>)">
        ...
        <div class="ev-legend">
            ...
            <span class="badge-status badge-past"><i class="fa-solid fa-clock-rotate-left"></i> Past Event</span>
        </div>
